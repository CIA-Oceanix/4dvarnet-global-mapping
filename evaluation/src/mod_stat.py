...
def compute_stat_scores_osse(ds_interp, lambda_min, lambda_max, output_file, method_name=' '):
    """
    Compute statistical scores for interpolation results.
    ...
    ----------
    ds_interp : xarray.Dataset
        Interpolated dataset containing 'sla_interpolated', 'sla_unfiltered', and 'msla_interpolated'.
    lambda_min : float
        Minimum wavelength for bandpass filter.
    lambda_max : float
        Maximum wavelength for bandpass filter.
    ...
    logging.info("Compute mapping error all scales")
    ds_interp['mapping_err'] = ds_interp['msla_interpolated'] - (ds_interp['sla_unfiltered'])
    
    
    print('Total RMSE=',np.sqrt(np.mean(ds_interp['mapping_err'].values**2)),'m')
    
    print('Total RMSE score=',1-np.sqrt(np.mean(ds_interp['mapping_err'].values**2))/np.sqrt(np.mean((ds_interp['sla_unfiltered']).values**2)),'m')
