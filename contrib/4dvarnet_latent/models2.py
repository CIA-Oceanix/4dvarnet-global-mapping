...
class GradModelWithCondition(torch.nn.Module):
    ...
    def forward(self, x, timesteps=None, extra=[]):
        ...
        x = self.dropout(x)
        #print("Timestep value range:", timesteps.min().item(), timesteps.max().item())
...
class GradSolver_withStep(GradSolver):
    ...
    def solver_step(self, state, batch, step, alpha_step=1.):
        ...
        if 'subgrad' in self.input_grad_update :
            ...
            var_cost = self.prior_cost(state) + self.lbd**2 * self.obs_cost(state, batch)
            grad = torch.autograd.grad(var_cost, state, create_graph=True)[0]
            print("Grad value range:", grad.min().item(), grad.max().item())
        ...
        print("Gmod value range:", gmod.min().item(), gmod.max().item())
    ...
    def forward(self, batch, x_init=None, h_state=None,phase='test'):
        ...
        with torch.set_grad_enabled(True):
            ...
            self.grad_mod.reset_state(batch.input)
            # print("\n===== forward_one_inference_step =====")
            # print("phase:", phase)
            # print("self.training:", self.training)
            # print("input_grad_update:", self.input_grad_update)
            # print("torch.is_grad_enabled():", torch.is_grad_enabled())
            # print("x_init.requires_grad:", state.requires_grad)
            # print("x_init.grad_fn:", state.grad_fn)
            # print("x_init.is_leaf:", state.is_leaf)
            for step in range(self.n_step):
                state_prev = state
                ...
                # print("state.is_leaf:", state.is_leaf)
                # print("state changed from prev:",(state is not state_prev))                
...
class GradSolver_FM(GradSolver_withStep):
    def __init__(self, prior_cost, obs_cost, grad_mod, back_cost, n_step,n_step_inference, lr_grad=0.2, lbd=1.0,std_init=0., **kwargs):
        """
        Initialize the GradSolver.
        ...
        self.std_init = std_init
        self.n_step_inference = n_step_inference
    ...
    def init_state(self, batch, x_init=None, std_init=0., use_fm_learning=False, step_init=None, phase='test'):
        ...
        t = torch.rand(1, device=batch.input.device).expand(batch.input.shape[0])
    ...
    def forward_one_inference_step(self, batch, x_init, alpha, beta, t_init, phase='test', use_fm_learning=False):
            with torch.set_grad_enabled('grad' in self.input_grad_update or phase == 'train'):
                if phase == 'train' and use_fm_learning:
                    alpha_step = (1. - t_init) / self.n_step
                    alpha_step = torch.tensor([alpha_step], device=x_init.device).repeat(x_init.shape[0])
                ...
                for step in range(self.n_step):
                    ...
                    state = self.solver_step(state, batch, x_init, alpha, beta, step = t_step , alpha_step=alpha_step)
                    
                    # ✅ CORRECTION : On ne détache QU'EN mode test/inférence pour économiser la mémoire.
                    # En entraînement, on doit garder le graphe actif !
                    if phase != 'train':
                        state = state.detach().requires_grad_(True)
    ...
    #     print("Current PHASE in GradSolver_FM forward:", phase, flush=True)
    #     if phase == 'test' and use_fm_learning: # ODE solver
    #         # sampling init state
    #         x0 = self.std_init * torch.randn( *batch.input.shape[0:],
    ...
    #         # ODE solver loop (ensure autograd is enabled so operations build the graph)
    #         with torch.set_grad_enabled(True):
    #             for ode_step in range(1,n_step_inference):
    #                 print('ODE step', ode_step, flush=True)
    #                 t  = ode_step * 1. / n_step_inference
    #                 t = torch.tensor([t], device=state.device).repeat(state.shape[0])
    ...
    #                 alpha_ = alpha.view(-1,1,1,1).repeat(1,batch.input.shape[1],batch.input.shape[2],batch.input.shape[3])
    #                 beta_ = beta.view(-1,1,1,1).repeat(1,batch.input.shape[1],batch.input.shape[2],batch.input.shape[3])
    #                 dalpha_ = torch.tensor([dalpha], device=state.device).view(-1,1,1,1).repeat(1,batch.input.shape[1],batch.input.shape[2],batch.input.shape[3]).requires_grad_(True)
    #                 dbeta_ = torch.tensor([dbeta], device=state.device).view(-1,1,1,1).repeat(1,batch.input.shape[1],batch.input.shape[2],batch.input.shape[3]).requires_grad_(True)


    #                 self.init_h_state(batch, h_state=None)
    #                 expected_mean, h_state = self.forward_one_inference_step(batch, state, alpha, beta, t, phase=phase,)
    #                 # keep expected_mean in the graph (do not detach)
    #                 expected_x0 = 1. / alpha_ * ( state - beta_ * expected_mean )        

    #                 state = state + (1. / n_step_inference) * ( dalpha_ * expected_x0 + dbeta_ * expected_mean )    
    ...
    #         self.init_h_state(batch, h_state=h_state)
    #         self.grad_mod.reset_state(batch.input)
    #         alpha, dalpha = self.alpha_step( t_init )
    #         beta, dbeta = self.beta_step( t_init )        
    #         state, h_state = self.forward_one_inference_step(batch, x_init,  alpha, beta, t_init=t_init, phase=phase, use_fm_learning=use_fm_learning)
    #         return state
    ...
    def forward(self, batch, x_init=None, h_state=None, phase='test', use_fm_learning=False):
            ...
            if n_step_inference > 1: # ODE solver
                # ✅ On force le mode grad si on est en train
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # ✅ En entraînement, x0 doit pouvoir porter des gradients si nécessaire
                    if phase != 'train':
                        state = state.detach().requires_grad_(True)
                    ...
                    for ode_step in range(1, n_step_inference):
                        ...
                        self.init_h_state(batch, h_state=None)
                        expected_mean, h_state = self.forward_one_inference_step(batch, state, alpha, beta, t, phase=phase, use_fm_learning=use_fm_learning)
                        expected_x0 = 1. / alpha * (state - beta * expected_mean)      
                        update = (1. / n_step_inference) * (
                            dalpha * expected_x0 + dbeta * expected_mean
                        )
                        
                        # ✅ CORRECTION : On écrase 'state' pour que l'étape suivante dépende de l'étape actuelle
                        state = state + update

                    return state # ✅ On retourne le state final chaîné
    ...
    def solver_step(self, state, batch, x_init, alpha, beta, step, alpha_step=1.):
        ...
        if( isinstance(step, float) ):
            t = torch.tensor([step], device=state.device).repeat(state.shape[0])
            alpha = torch.tensor([alpha], device=state.device).repeat(state.shape[0])
            beta = torch.tensor([beta], device=state.device).repeat(state.shape[0])
        else:
            t = step
        ...
        if 'subgrad' in self.input_grad_update :
            gobs = (batch.input-(x_mean+state)).nan_to_num()
            if beta > 0.:
                gback = beta**2/self.alpha**2 * (beta**(-1)*x_init-state)
            else:
                gback = torch.zeros_like(x_init)
            gprior = state - self.prior_cost.forward_ae(state)
            grad = torch.concatenate((self.format2D_3D(gobs),self.format2D_3D(gprior),self.format2D_3D(gback)),dim=1)
            ...
            prior_cost = self.prior_cost(state)
            obs_cost = self.obs_cost(state, batch)
            back_cost = self.back_cost(state, x_init, alpha, beta)
            # Compute full gradient
            grad_prior = torch.autograd.grad(prior_cost, state, create_graph=True)[0]
            grad_obs = torch.autograd.grad(obs_cost, state, create_graph=True)[0]
            grad_back = torch.autograd.grad(back_cost, state, create_graph=True)[0]
            grad = torch.concatenate((grad_prior, grad_obs, grad_back),dim=1)
            if 'state' in self.input_grad_update :
                grad = grad / ((grad**2).mean().sqrt().detach())
                grad = torch.concatenate((grad,state),dim=1)   
            ...
            #prior_cost = self.prior_cost(state)
            #obs_cost = self.obs_cost(state, batch)
            var_cost = self.prior_cost(state) + self.lbd**2 * self.obs_cost(state, batch) + self.back_cost(state, x_init, alpha, beta)
            grad = torch.autograd.grad(var_cost, state, create_graph=True)[0]
            ...
            grad = torch.concatenate((self.format2D_3D(state),self.format2D_3D(batch.input.nan_to_num()), self.format2D_3D(x_init)),dim=1)
...
#############################################
# 4DVarNet-FM sampler for generating samples from a pretrained model
############################################# 
...
class GradSolver_FM_anomaly(GradSolver_withStep):
    def __init__(self, prior_cost, obs_cost, grad_mod, back_cost, n_step, n_step_inference, lr_grad=0.2, lbd=1.0,std_init=0., **kwargs):
        """
        Initialize the GradSolver.
    ...
    def init_state(self, batch, x_init=None, std_init=0., step_init=None, phase='test'):
        """
        Initialize the state for optimization.
    ...
    def forward(self, batch, solver_pretrained, x_init=None, h_state=None, phase='test', use_fm_learning=False):
        """
        Perform the forward pass of the solver.
        ...
        #Mean state from pretrained model
        x_mean = solver_pretrained(batch)
        if n_step_inference > 1: # ODE solver
            # ✅ On force le mode grad si on est en train
            with torch.set_grad_enabled(phase == 'train'):
                ...
                # ODE solver loop
                for ode_step in range(1, self.n_step_inference):
                    print('ODE step', ode_step, flush=True)
                    t  = (ode_step * 1.) / n_step_inference
                    alpha, dalpha = self.alpha_step( t )
                    beta, dbeta = self.beta_step( t )
                    kappa = beta*(beta**2 + alpha**2)**(-1)
                    expected_mean = self.forward_one_inference_step(batch, state, t, x_mean, alpha, beta, kappa, phase=phase)
                    expected_x0 = 1. / alpha * (state - beta * expected_mean)     
                    update = (1. / n_step_inference) * (
                           dalpha * expected_x0 + dbeta * expected_mean
                       )
    ...
    def forward_one_inference_step(self, batch, x_init, t_init, x_mean,alpha, beta, kappa, phase='test', use_fm_learning=True):
        with torch.set_grad_enabled( 'grad' in self.input_grad_update or phase == 'train' ):
            if phase == 'train' and use_fm_learning:
                alpha_step = (1. - t_init) / self.n_step
                alpha_step = torch.tensor([alpha_step], device=x_init.device).repeat(x_init.shape[0])
            ...
            state = kappa * x_init + (1 - kappa) * x_mean
            for step in range(self.n_step):
                t_step = t_init + ( step / self.n_step ) * ( 1. - t_init )
                state = self.solver_step(state, batch, x_mean=x_mean, x_init=x_init,alpha=alpha, beta=beta, step = t_step , alpha_step=alpha_step)
                if phase != 'train':
                    state = state.detach().requires_grad_(True)
    ...
    def solver_step(self, state, batch, x_mean, x_init, alpha, beta, step, alpha_step=1.):
        """Perform one step of the solver.
        """
        ...
        if( isinstance(step, float) ):
            t = torch.tensor([step], device=state.device).repeat(state.shape[0])
            alpha_ = torch.tensor([alpha], device=state.device).repeat(state.shape[0])
            beta_ = torch.tensor([beta], device=state.device).repeat(state.shape[0])
        else:
            t = step
        #print(t)    
        ...
        if 'subgrad' in self.input_grad_update :
            ...
            var_cost = self.prior_cost(state) + self.lbd**2 * self.obs_cost(state, batch) + self.back_cost(state, x_init, alpha_, beta_)
            grad = torch.autograd.grad(var_cost, state, create_graph=True)[0]
        ...
        self.h_state = state_update
        print('state update require grad', state_update.requires_grad)
        return state - alpha * beta * state_update

class BackgroundCost(nn.Module):
    def __init__(self, w=1) -> None:
        super().__init__()
        self.w=w

    def forward(self, state, x_tau, alpha, beta):
        if not torch.is_tensor(beta) and beta == 0.:
            return torch.tensor(0., device=state.device)
        beta = beta.view(-1,1,1,1).repeat(1,state.shape[1],state.shape[2],state.shape[3])
        alpha = alpha.view(-1,1,1,1).repeat(1,state.shape[1],state.shape[2],state.shape[3])
        x_tau = beta**(-1)*x_tau
        back_cost = self.w * torch.nn.functional.mse_loss(x_tau, state,weight=beta**2/alpha**2)
        return back_cost
