...
class Lit4dVarNet(pl.LightningModule):
    ...
    def weighted_mse(err, weight):
        print(f"err shape: {err.shape}, weight shape: {weight.shape}")
...
class GradSolver(nn.Module):
    ...
    def forward(self, batch, init_state=None):
        with torch.set_grad_enabled(True): # To decide how to swith between the two cases of state
...
class ConvLstmGradModel(nn.Module):
    def __init__(self, dim_in, dim_hidden, kernel_size=5, dropout=0.1, downsamp=None):
        super().__init__()
...
class BilinAEPriorCost(nn.Module):
    def __init__(self, dim_in, dim_hidden, kernel_size=5, downsamp=None, bilin_quad=True):
        super().__init__()
...
class LitUnet_like_4dVarNet(Lit4dVarNet):
    """Lightning module for a UNet-like model in the 4dvarnet setup, where there is not prior cost or ConvLSTM,
    grad solver is simply replaced by a Unet mapping the input to output."""
    def __init__(self, solver, rec_weight, opt_fn, test_metrics=None, pre_metric_fn=None, norm_stats=None, persist_rw=True):
